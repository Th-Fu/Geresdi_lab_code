    ...
    def __init__(self, filename):
        ...
        with open(filename, 'r') as f:
            for line in f:
                ...
                elif line.startswith('#\tdo_times'):
                    do_times = float( line.split(': ', 1)[1])
                    self.do_times[self.columns[-1]] = do_times
...
#                min_idx = np.ceil((np.nanmin(column.values) - minimum) / diff)
#                max_idx = np.ceil((np.nanmax(column.values) - minimum) / diff)
...
def create_kernel(x_dev, y_dev, cutoff, distr):
    ...
    hx = np.int(np.floor((x_dev * cutoff) / 2.0))
    hy = np.int(np.floor((y_dev * cutoff) / 2.0))
