...
class mw_switch_class(d5a_module):
    ...
    def close_port(self, port: int):
        ...
        print(f"Closing port {port}, voltage provided by: {dac}")
        dac.voltage(4)
    ...
    def open_port(self, port: int):
        ...
        if port > 6 or port < 1:
            raise TypeError(f"Port should be inclusive between 1 and 6")   
        dac = self.dac_list[port-1]
        print(f"Opening port {port}, reducing: {dac} to 0V")
        dac.voltage(0)
    ...
    def closed_port_list(self):
        ...
        for dacco in self.dac_list:
            ...
            if float(check_val) != float(0.0):
                closed_ports.append(f"port {counter+1}")
            if float(check_val) < 0.0:
                print("Excuse me? Please only apply positive voltages on the dacs for the switching purpose...")
                raise ValueError 
            
            counter +=1
        if len(closed_ports) == 0:
            print("No closed ports! (i.e. no applied voltages)")
        elif len(closed_ports) == 1:
            print(f"The only closed port is {closed_ports[0]}.")
        else:
            print(f"The closed ports are {', '.join(closed_ports)}.")
