...
class calibration(object):
    ...
    # functions from https://github.com/charlesll/rampy/blob/master/rampy/baseline.py

    def _baseline_arPLS(self, y , lam = 1e5, ratio = 0.01):
    ...
    def fit_baseline_arPLS(self, y, lam = 1e5, ratio = 0.01):
        '''
        for this to work, you need to analyze a large part of the baseline
        tune lam and p until you get the desired result
        '''
        return self._baseline_arPLS(np.absolute(y),lam,ratio)

    def baseline_func_arPLS(self, y, lam = 1e5, ratio = 0.001, eta = 0.5, niter = 100):
        '''
        for this to work, you need to analyze a large part of the baseline
        tune lam and p until you get the desired result
        ...
        '''
        return interp1d(f_data, self._baseline_arPLS(lam = 1e6, ratio = 0.01), kind='cubic')
    ...
    def _baseline_drPLS(self, y, lam , ratio, eta , niter):
        #according to Applied Optics, 2019, 58, 3913-3920.
	#y, lam = 1e6, ratio = 0.001, eta = 0.5, niter = 100
        #optional parameters
    ...
    def fit_baseline_drPLS(self, y, lam = 1e6, ratio = 0.001, eta = 0.5, niter = 100):
        '''
        for this to work, you need to analyze a large part of the baseline
        tune lam and p until you get the desired result
        '''
        return self._baseline_drPLS(np.absolute(y),lam,ratio,eta, niter=niter)

    def baseline_func_drPLS(self, y, lam = 1e6, ratio = 0.001, eta = 0.5, niter = 100):
        '''
        for this to work, you need to analyze a large part of the baseline
        tune lam and p until you get the desired result
        returns the baseline as a function
        the points in between the datapoints are computed by cubic interpolation
        '''
        return interp1d(f_data, self._baseline_drPLS(np.absolute(y),lam,ratio,eta ,niter=niter), kind='cubic')
    
    
    
    ##### AsLS smooth
    
    def _baseline_als(self,y, lam = 1e6, p = 0.95, niter=10, weights = False):
        '''
    ...
    def fit_baseline_als(self,z_data,lam,p,niter=10):
        '''
        for this to work, you need to analyze a large part of the baseline
        tune lam and p until you get the desired result
        '''
    ...
    def baseline_func_als(self,z_data,f_data,lam,p,niter=10):
        '''
        for this to work, you need to analyze a large part of the baseline
        tune lam and p until you get the desired result
        returns the baseline as a function
